...
// Video
var $video = $("#myVideo");
var $videoContainer = $(".page_content");
...
/*------- INSERTAR BOTON PLAY DEL INICIO ----*/
...
$video.on("timeupdate", function() {
	...
	if ($videoTime < 9.5) {
		$duration.html("0" + Math.round($videoTime) + "s" + " / " + parseInt($video[0].duration) + "s");		
	} else {
		$duration.html( Math.round($videoTime) + "s" + " / " + parseInt($video[0].duration) + "s");			
	}
});
...
$videoContainer.on("mouseleave", function() {
	if($video[0].paused === false) {
		/*$buttonControls.hide();*/
		$videoControls.css("margin-bottom", "0");	  
	}
});
